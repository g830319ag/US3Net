...
class Trainer(nn.Module):
    ...
    def compute_occlusion_mask(self, d_left, gamma=2.5):
        assert not torch.isnan(d_left).any(), "d_left contains inf values!"
        B, _, H, W = d_left.size()
        device = d_left.device
        d_left_int = d_left.clamp(min=0).long()
        
        # Right image coordinates
        right_u = torch.arange(W, device=device, dtype=torch.long).view(1, 1, 1, W).repeat(B, 1, H, 1) - d_left_int
        right_u = right_u.clamp(min=0, max=W-1)
        right_v = torch.arange(H, device=device, dtype=torch.long).view(1, 1, H, 1).repeat(B, 1, 1, W)
        right_indices = (right_v * W + right_u).view(B, -1)
        
        # Flatten disparity map
        d_flat = d_left.view(B, -1)

        # Use scatter_max for batch operation
        right_max_d = torch.full((B, H * W), -float('inf'), device=device)
        right_max_d, _ = scatter_max(d_flat, right_indices, dim=1, dim_size=H * W)
        right_max_d = right_max_d.view(B, H, W)
        
        # Compute occlusion mask
        right_max_at_left = right_max_d.gather(-1, right_u.view(B, H, W))
        right_max_at_left = right_max_at_left.clamp(min=0)
        diff = torch.abs(right_max_at_left - d_left.squeeze(1))

        w = torch.special.expit((right_max_at_left - 10))
        occlusion = torch.exp(-gamma*w*diff)
    ...
    def compute_grad2_smoothness_loss(self, flo, image, beta):
        ...
        dx2, _ = self.gradient(dx)
        _, dy2 = self.gradient(dy)
        
        return (torch.mean(beta*weights_x[:,:, :, 1:]*torch.abs(dx2)) + torch.mean(beta*weights_y[:, :, 1:, :]*torch.abs(dy2))) / 2.0

    # def reconstruction_loss(self, x, y, mask):
    #     ssim = torch.mean(self.SSIM(x, y)*mask)
    #     l1 = torch.mean(torch.abs(x - y)*mask)
    ...
    def compute_loss(self, imgL, imgR, epoch):
        ...
        for l_disp, r_disp_ast in zip(l_disps, r_disps_ast):
            ...
            if epoch > self.only_recon_epoch:
                left_occlusion_mask = self.compute_occlusion_mask(l_disp, gamma=2.5)
            else:
                left_occlusion_mask = torch.ones_like(recon_imgL)
            left_recon_loss = self.reconstruction_loss(recon_imgL[:,:,:, 75:575], imgL[:,:,:, 75:575], left_occlusion_mask[:,:,:, 75:575]) 
            # left_recon_loss = self.reconstruction_loss(recon_imgL[:,:,:, 75:575], imgL[:,:,:, 75:575])
            left_recon_loss = torch.mean(left_recon_loss)
            # compute right reconstruction loss
            ...
            if epoch > self.only_recon_epoch:
                right_occlusion_mask = self.compute_occlusion_mask(r_disp_ast, gamma=2.5).flip(3)
            else:
                right_occlusion_mask = torch.ones_like(recon_imgR) 
            right_recon_loss = self.reconstruction_loss(recon_imgR[:,:,:, 0:500], imgR[:,:,:, 0:500], right_occlusion_mask[:,:,:, 0:500])
            # right_recon_loss = self.reconstruction_loss(recon_imgR[:,:,:, 0:500], imgR[:,:,:, 0:500])
            right_recon_loss = torch.mean(right_recon_loss) 
            ...
            if epoch > self.only_recon_epoch:
                ...
                # compute left-right consistency loss (disparity)
                ...
                # lr_right_loss = torch.mean(torch.abs(l_disp[:, :, :, 75:575] - r2l_disp[:, :, :, 75:575]) * left_occlusion_mask[:, :, :, 75:575])
                # lr_left_loss = torch.mean(torch.abs(r_disp[:, :, :, 0:500] - l2r_disp[:, :, :, 0:500]) * right_occlusion_mask[:, :, :, 0:500])
