...
def test_single_pair(model,imgL,imgR, use_cuda):
    ...
    #pred_disp = np.where(pred_disp < 0, 0, pred_disp)
    return pred_disp
...
def save_pred_disp_split_dir(pred_disp: np.ndarray, save_path, basename):
    ...
    plt.imsave(os.path.join(save_path, 'plot', basename+'.png'), pred_disp, cmap='jet')
    # pred_disp = (pred_disp*256).clip(min=0).astype('uint16')
    # pred_disp = Image.fromarray(pred_disp)
    # pred_disp.save(os.path.join(save_path,'test', basename+'.png'))
    # plt.imsave(os.path.join(save_path, 'plot', basename+'.png'), pred_disp, cmap='plasma')
...
if __name__ == '__main__':
    ...
    if MULTI_PAIRS:
        ...
        print(f'all estimated disparity maps are saved at {args.save_path}!')