...
class featexchange(nn.Module):
    def __init__(self):
        ...
        self.x2_fusion = nn.Sequential(nn.ReLU(),convbn(4, 4, 3, 1, 1, 1, 4),
                                       nn.ReLU(),
                                       convbn(4, 4, 3, 1, 1, 1, 4))
        self.upconv4 = nn.Sequential(nn.Conv2d(2, 4, 1, 1, 0, bias=False),
                                     norm_layer2d(4))
        self.upconv8 = nn.Sequential(nn.Conv2d(1, 4, 1, 1, 0, bias=False),
                                     norm_layer2d(4)) 
        
        
        self.x4_fusion = nn.Sequential(nn.ReLU(),convbn(8, 8, 3, 1, 1, 1, 8),
                                       nn.ReLU(),convbn(8, 8, 3, 1, 1, 1, 8))
        self.downconv4 = nn.Sequential(DepthwiseSeparableConv(4, 8, 3, 2, 1),
                                       norm_layer2d(8))
        self.upconv8_2 = nn.Sequential(nn.Conv2d(4, 8, 1, 1, 0, bias=False),
                                     norm_layer2d(8))

        self.x8_fusion = nn.Sequential(nn.ReLU(),convbn(16, 16, 3, 1, 1, 1, 16),
                                       nn.ReLU(),convbn(16, 16, 3, 1, 1, 1, 16))
        self.downconv81 = nn.Sequential(DepthwiseSeparableConv(8, 16, 3, 2, 1),
                                       norm_layer2d(16))
        self.downconv82 = nn.Sequential(DepthwiseSeparableConv(8, 16, 3, 2, 1),
                                       norm_layer2d(16))
        
    def forward(self, x2, x4, x8, attention):
        A = torch.split(attention,[4,8,16],dim=1)
        x4tox2 = self.upconv4(F.pixel_shuffle(x4, 2)) # 1/8 -> 1/4
        x8tox2 = self.upconv8(F.pixel_shuffle(x8, 4)) # 1/16 -> 1/4
        #fusx2  = self.conv1x1(x2) + x4tox2 + x8tox2 # 1/2
        fusx2 = x2 + x4tox2 + x8tox2 # channel: 56
        fusx2  = self.x2_fusion(fusx2)*A[0].contiguous()+fusx2

        x2tox4 = self.downconv4(x2)
        x8tox4 = self.upconv8_2(F.pixel_shuffle(x8, 2))
...
class feature_extraction(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Sequential(nn.Conv2d(48, 12, 1, 1, 0, bias=False),
                                   norm_layer2d(12),
                                   nn.ReLU(),
                                   nn.Conv2d(12, 4, 1, 1, 0, bias=False),
                                   #DepthwiseSeparableConv(4, 4, 3, 1, 1),
                                   norm_layer2d(4),
                                   nn.ReLU(),
                                   DepthwiseSeparableConv(4, 4, 3, 1, 1),
                                   norm_layer2d(4),
        ...
        self.stage2 = nn.Sequential(DepthwiseSeparableConv(4, 8, 3, 2, 1),
                                    norm_layer2d(8),
                                    nn.ReLU(),
                                    DepthwiseSeparableConv(8, 8, 3, 1, 1),
                                    norm_layer2d(8),
                                    nn.ReLU()) # 1/8

        self.stage3 = nn.Sequential(DepthwiseSeparableConv(8, 16, 3, 2, 1),
                                    norm_layer2d(16),
                                    nn.ReLU(),
                                    DepthwiseSeparableConv(16, 16, 3, 1, 1),
                                    norm_layer2d(16),
                                    nn.ReLU()) #1/16
                
        self.stage4 = nn.Sequential(nn.AdaptiveAvgPool2d(1),
                                    nn.Conv2d(16, 10, 1, 1, 0, bias=True),
                                    nn.ReLU(),
                                    nn.Conv2d(10, 28, 1, 1, 0, bias=True),
                                    nn.Sigmoid(),
                                    ) 
...
class Pseudo3DCNN(nn.Module):
    def __init__(self, out_channels, kernel_size, stride, padding):
        super(Pseudo3DCNN, self).__init__()
        # Spatial (height and width) dimensions convolution
        ...
        # Temporal (disparity) dimension convolution
        ...
        self.bn3d1 = nn.BatchNorm3d(out_channels)
        self.relu1 = nn.ReLU()
...
def post_pseudo3dconvs(layers, channels, kernel_size=(3, 3, 3), stride=(1, 1, 1), padding=(1, 1, 1)):
    net  = [nn.Conv3d(1, channels, kernel_size=3, padding=1, stride=1, bias=False)]
    for _ in range(layers):
        net.append(Pseudo3DCNN(channels, kernel_size, stride, padding))
    net.append(batch_relu_conv3d(channels, 1))
...
class US3Net(nn.Module):
    def __init__(self, maxdisp, multi_scale=False):
        super(US3Net, self).__init__()
...
if __name__ == '__main__':
    model = US3Net(192).cuda()
    print('Number of model parameters: {}'.format(sum([p.data.nelement() for p in model.parameters()])))
    input = torch.randn(1, 3, 288, 576).cuda()
